...
class CanadaPaymentFactory(PaymentFactory):
    def __init__(self):
        self.payments = dict(
            credit_card=CreditCard, netbanking=NetBanking, paypal=PayPal
        )
...
class NigeriaPaymentFactory(PaymentFactory):
    def __init__(self):
        self.payments = dict(
            credit_card=CreditCard, remita=Remita, netbanking=NetBanking
        )
...
class IndiaPaymentFactory(PaymentFactory):
    def __init__(self):
        self.payments = dict(
            credit_card=CreditCard, debit_card=DebitCard, netbanking=NetBanking
        )


# list of payemnts methods allwodwd based on country
class Client:
    def __init__(self):
        self.factory = None
    ...
    def get_factory(self):
        ...
        # no factory get list of Factory
    ...
    def do_payment(self):
        if self.factory:
            ...
            available_payments = ", ".join(
                self.factory.payments
            )  # e.g if US = credit_card, debit_card, paypal
            payment_method = input(
                f"Enter Your Payment Method: ({available_payments}): "
            )
            if payment_method in self.factory.payments:
                payment = self.factory.payments.get(payment_method)()
                payment.make_payment(amount)
            else:
                print("PAYMENT ERROR: You cannot use this type of payment")
        else:
...
# Easily Extend the code another payments methods comes as part of payemnts
