...
class DatabaseConnection:
    ...
    def insert(self, emp):
        return self.query(
            "INSERT INTO employee VALUES(?,?,?)", (emp.firstname, emp.lastname, emp.age)
        )

    def release(self):
        # Reset connection state if necessary
        # For example, closing the cursor and
...
class DatabaseConnectionPoolManager:
    def __init__(self, max_connections):
        ...
        for _ in range(max_connections):
            # all available connection have only one connecton self.connection
            # all connection pointing to single Instances
...
print(
    f"Available : {len(pool.available_connections)}, In Use: {len(pool.in_use_connections)}"
)
